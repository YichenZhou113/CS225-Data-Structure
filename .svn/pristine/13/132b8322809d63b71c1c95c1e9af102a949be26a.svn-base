...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         */
         ...
         string customerurl = ll.customer + ll.url;
         int check = 0;
         for(size_t i = 0; i < uniqueURLs.size(); i++){
           if(uniqueURLs[i] == ll.url){check = 1;}
         }
         if(check == 0){
           uniqueURLs.push_back(ll.url);
         }
         if(whenVisitedTable.keyExists(customerurl)){
           if(ll.date > whenVisitedTable[customerurl]){
             whenVisitedTable[customerurl] = ll.date;
           }
         }
         else{
           whenVisitedTable.insert(customerurl, ll.date);
         }
    }
    infile.close();
}
