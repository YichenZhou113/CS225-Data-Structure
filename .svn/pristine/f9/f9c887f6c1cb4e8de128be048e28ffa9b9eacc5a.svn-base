...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for(int i = 0; i<int(frequencies.size()); i++){
       TreeNode *n = new TreeNode(frequencies[i]);
       singleQueue.push(n);
     }

       while(singleQueue.size()+mergeQueue.size()>1){
         TreeNode *one = removeSmallest(singleQueue,mergeQueue);
         TreeNode *two = removeSmallest(singleQueue,mergeQueue);
         TreeNode *newin = new TreeNode(one->freq.getFrequency()+two->freq.getFrequency());
         newin->left = one;
         newin->right = two;
         mergeQueue.push(newin);
       }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if(bfile.getNextBit()==1){
       TreeNode *n1 = new TreeNode(Frequency(bfile.getNextByte(),0));
       return n1;
     }else{
       TreeNode *n2 = new TreeNode(0);
       n2->left = readTree(bfile);
       n2->right = readTree(bfile);
       return n2;
     } // replaceme!
}
